export async function handle(state, action){
   ...
   // CONSTANTS
    ...
    const ERROR_TRANSFERING_CURRENT_LABEL = "you have to switch the label from currentLabel to ownedLabels before making the transfer";
    const ERROR_INVALID_TARGET_TRANSFER = "the target is not a valid arweave address";
    ...
    if (input.function === "transfer") {
        ...
        const desiredLabel = _validateUsername(label, "read");
        const {ownedIn, callerIndex} =  _getTransferablityStatus(desiredLabel, caller);


        const callerProfile = users[callerIndex];
        const isTargetRegistered = users.find(usr => usr.user === target);
        const labelObject = callerProfile["ownedLabels"].find(labels => labels.label === desiredLabel);
        ...
        if (ownedIn === "ownedLabels") {
            ...
            const labelIndexInOwnedLabels = callerProfile["ownedLabels"].findIndex(labels => labels.label === desiredLabel);
    ...
    function _validateLabelSwitching(label, address) {
        ...
        if (! callerProfile["ownedLabels"].find(labels => labels.label === label)) {
    ...
    function _getTransferablityStatus(label, address) {
        ...
        if ( (callerProfile["currentLabel"] === label) && (callerProfile["ownedLabels"].length > 1) ) {
